...
describe('GET /api/v1/messages', () => {
  describe('should fetch all received mails', () => {
    it('on sucess it should return an object with properties status and data', (done) => {
      chai.request(server)
        .get('/api/v1/messages')
        .set('Cookie', 'jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MSwiaWF0IjoxNTUzODcxOTg1LCJleHAiOjE1ODU0MDc5ODV9.kxiPZIp8pU9_MlvdvPaosR7GYwtjh865tP7myxsuvnw')
        .end((err, res) => {
          // eslint-disable-next-line no-unused-expressions
          expect(err).to.be.null;
          expect(res, 'response object status').to.have.status(200);
...
describe('DELETE /api/v1/messages/<messages-id>', () => {
  ...
  after((done) => {
    ...
    db.query(query1, value1)
      .then(() => {
        db.query(query2, value2)
          .then(() => {
            db.query(query3, value3);
          });
      });
    done();
  });
